...
The C APIs follows current C++ API implementation, `TF_Buffer` and related proto files are the interface between proper and plugin.
The graph C APIs can be found in `tensorflow/c/experimental/grappler/grappler.h`.
...
* `GraphDef`, `NodeDef`: These are C++ objects generated by protobuf toolchain with a predefined structure in graph.proto. Plugin should maintain protobuf toolchain and the object itself, with a same proto file as defined in Tensorflow proper.

* `BufferToMessage`, `MessageToBuffer`: They are serialization/deserialization function between `TF_Buffer` and protobuf objects(`GraphDef`). Plugin can deserialize input graph to plugin GraphDef onject, and serialize the output GraphDef object when graph transformation is finished.
...
Modular TensorFlow provides some util C APIs to help retrieve necessary graph information:
1.  `TF_GrapplerItem` represents a combination of a graph, and some more information about feed/fetch nodes, preserved nodes. Here is an example of how to get a set of preserved nodes.
...
2. `TF_GraphProperties` can be used to infer OpInfo::TensorProperties. Typical use case is to first call `TF_InferStatically` to statically infer shapes and then call `TF_GetInputPropertiesList` to get input shapes.
...
3. `TF_FunctionLibraryDefinition` maintains a map between op names and op definitions, typical use case is to look up an OpDef by op name, and then get some op attributes.
